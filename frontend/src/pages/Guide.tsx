...
const SECTIONS: { id: string; title: string; icon: string; content: string[] }[] = [
  ...
  },
  {
    ...
    "icon": "🔌",
    "content": [
      "进入系统配置 → 数据源配置。每个数据源卡片会说明来源、是否需要 Token、支持哪些能力，以及当前是启用、停用还是暂不可用。",
      "按能力测试：在路由表中选择“大盘指数、个股行情、日K、财务、资金流、股东户数、龙虎榜”等具体能力，再点击“测试连接”。免费来源可以直接测试；需要凭证的来源，输入新 Token 会用新 Token 临时测试，输入框留空则使用已保存的加密凭证。",
      "路由模式：自动模式会按首选→备用顺序切换；固定模式只保留一个来源。可以用上移/下移或勾选候选来源调整顺序，保存后下一次行情请求立即按服务端真实配置取数。",
      "安全提醒：Token 只在服务端加密保存，页面和日志不会显示完整凭证；保存或测试后输入框会清空。不要把 Token 写进备注、截图或报告。",
      "状态说明：fresh 表示在当前能力时效内，stale 表示返回最近一次有效数据并标注数据时间，503 表示当前来源和备用来源都不可用，需要稍后重试或检查凭证。"
    ]
  },
  {
    "id": "membership",
