...
| LLM | DeepSeek API（默认），可切 Kimi/通义 | 便宜、中文金融文本能力强；统一封装一层便于切换 |
...
├── core/           # 配置、日志、鉴权工具、LLM 客户端封装
└── main.py         # FastAPI 入口
```

**分层的意义**：接口层只负责"收发"，业务层负责"规矩"，AI 层负责"动脑"，数据层负责"取数"。以后换数据源只改 `datasource/`，换模型只改 `core/llm.py`，互不影响。
...
- 要求 LLM 输出 JSON，用 Pydantic 校验，校验失败自动重试 1 次再降级为"部分报告"。
...
| 新闻采集 | 每 10-30 分钟 | RSS/爬虫 → 去重 → LLM 标注利好利空与相关行业 |
...
股票分析、批量分析（最多 50 只，实测限制）、主力选股、持仓诊断、风险分析、龙虎榜分析——全部走"投递 → Worker 执行 → 落库 → 前端轮询"。任务台账 `task_records` 记录状态机：`pending → running → success / failed`，失败带错误信息。
...
| 密钥 | LLM Key、支付密钥只放服务器 `.env`，不进 git |
