...
suggestions(数组：编号式投资建议，每条含编号和建议文字),
summary(总体建议)"""
...
async def _safe_chat(messages: list[dict], user_id: int, module: str) -> dict:
    try:
        ...
        logger.warning(f"Portfolio diagnosis failed: {e}")
        return {"error": str(e)}


async def run_portfolio_diagnosis(holdings: list[dict], user_id: int, task: TaskRecord, db) -> dict:
    ...
    diagnosis = await _safe_chat(_build_diagnosis_prompt(data), user_id, "portfolio:diagnosis")
