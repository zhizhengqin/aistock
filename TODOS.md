...
- `[x]` 数据源与开盘啦完整修复（2026-08-24 Build）：已按 TDD 补齐股票估值、独立 `stock.profile` 公司资料、资金流回退与缺失质量、独立开盘啦原生能力、结构化凭证说明和逐卡片测试结果；后端/前端协议夹具及桌面和 390px E2E 已覆盖
- `[ ]` 开盘啦真实账号验收与剩余原生动作补齐
  - What：使用管理员自己的合法 `UserID + Token` 运行四类原生能力的 live-smoke，并继续确认涨停梯队、原生集合竞价等尚无可靠公开参数的动作。
  - Why：脱敏夹具只能证明代码转换逻辑，不能证明开盘啦当前成功响应仍与历史社区协议一致；未完成前不能标记为生产连接已通过。
  - Context：本次 Build 将独立建模个股盘口题材标签、板块排行、板块成分和股票排行，空凭证只会得到“未登录”；真实凭证不得写入仓库、日志、截图或测试夹具。
  - Depends on / blocked by：用户提供自己账户的合法开盘啦 `UserID + Token`，并确认调用符合其账户权限与服务条款；从 `backend/app/datahub/providers/kpl_native.py` 和对应 live-smoke 测试继续。
...
- `[x]` 生产数据可用性返修：快照、财务、资金流和新闻独立降级并标记告警，K 线仍为最低必需输入
- `[x]` Chief 输出兼容性返修：严格规范化合法数值入场区间/止盈值为字符串，继续拒绝非法范围与非有限值
